...
export const modelSelectionCheck: Check = {
  ...
  fixPrompt: (_ctx, result) =>
    `${result.message}. Sonnet handles 90%+ of coding tasks at ~5x lower cost than Opus. ` +
    `Set "model": "claude-sonnet-4-6" in ~/.claude/settings.json. ` +
    `If CLAUDE_MODEL or ANTHROPIC_MODEL is set in the shell profile, update it to claude-sonnet-4-6. ` +
    `For Cursor, update defaultModel in ~/.cursor/settings.json. Use /model opus selectively for complex architecture and planning.`,
